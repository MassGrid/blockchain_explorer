...
function get_summary(coin, exchange, cryptopia_id, cb) {
    ...
    request({ uri: base_url + '/GetMarket/' + cryptopia_id + '/24', json: true }, function (error, response, body) {
        if (error) {
            ...
            summary['bid'] = body.Data['BidPrice'].toFixed(3);
            summary['ask'] = body.Data['AskPrice'].toFixed(3);
            summary['volume'] = body.Data['Volume'];
            summary['high'] = body.Data['High'].toFixed(3);
            summary['low'] = body.Data['Low'].toFixed(3);
            summary['last'] = body.Data['LastPrice'].toFixed(3);
        ...
        if (body.Success == true) {
            ...
            for (var i = 0; i < tTrades.length; i++) {
                var Trade = {
                    ...
                    amount: parseFloat(tTrades[i].Amount).toFixed(3),
                    price: parseFloat(tTrades[i].Price).toFixed(3),
                    //  total: parseFloat(tTrades[i].Total).toFixed(3)
                    // Necessary because API will return 0.00 for small volume transactions
                    total: (parseFloat(tTrades[i].Amount).toFixed(3) * parseFloat(tTrades[i].Price)).toFixed(3),
        ...
        if (body.Success == true) {
            ...
            if (orders['Buy'].length > 0){
                for (var i = 0; i < orders['Buy'].length; i++) {
                    var order = {
                        amount: parseFloat(orders.Buy[i].Volume).toFixed(3),
                        price: parseFloat(orders.Buy[i].Price).toFixed(3),
                        //  total: parseFloat(orders.Buy[i].Total).toFixed(3)
                        // Necessary because API will return 0.00 for small volume transactions
                        total: (parseFloat(orders.Buy[i].Volume).toFixed(3) * parseFloat(orders.Buy[i].Price)).toFixed(3)
                    }
                    buys.push(order);
                }
                } else {}
                ...
                for (var x = 0; x < orders['Sell'].length; x++) {
                    var order = {
                        amount: parseFloat(orders.Sell[x].Volume).toFixed(3),
                        price: parseFloat(orders.Sell[x].Price).toFixed(3),
                        //    total: parseFloat(orders.Sell[x].Total).toFixed(3)
                        // Necessary because API will return 0.00 for small volume transactions
                        total: (parseFloat(orders.Sell[x].Volume).toFixed(3) * parseFloat(orders.Sell[x].Price)).toFixed(3)
                    }
                    sells.push(order);
                }
            } else {
