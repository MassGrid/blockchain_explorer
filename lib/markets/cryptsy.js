...
function get_summary(coin, exchange, Crymktid, cb) {
  ...
  request({uri: base_url + '/' + Crymktid + '/ticker', json: true}, function (error, response, body) {
    if (error) {
      ...
      summary['bid'] = body.data['bid'].toFixed(3);
      summary['ask'] = body.data['ask'].toFixed(3);
      ...
      var sells = [];
            if (orders['buyorders'].length > 0){
                for (var i = 0; i < orders['buyorders'].length; i++) {
                    var order = {
                        amount: parseFloat(orders.buyorders[i].quantity).toFixed(3),
                        price: parseFloat(orders.buyorders[i].price).toFixed(3),
                        //  total: parseFloat(orders.buyorders[i].total).toFixed(3)
                        // Necessary because API will return 0.00 for small volume transactions
                        total: (parseFloat(orders.buyorders[i].quantity).toFixed(3) * parseFloat(orders.buyorders[i].price)).toFixed(3)
                    }
                    buys.push(order);
                }
                } else {}
                ...
                for (var x = 0; x < orders['sellorders'].length; x++) {
                    var order = {
                        amount: parseFloat(orders.sellorders[x].quantity).toFixed(3),
                        price: parseFloat(orders.sellorders[x].price).toFixed(3),
                        //    total: parseFloat(orders.sellorders[x].total).toFixed(3)
                        // Necessary because API will return 0.00 for small volume transactions
                        total: (parseFloat(orders.sellorders[x].quantity).toFixed(3) * parseFloat(orders.sellorders[x].price)).toFixed(3)
                    }
                    sells.push(order);
                }
            } else {
