...
function get_summary(coin, exchange, cb) {
    ...
    request({ uri: base_url + 's.html?a=volume&h=24&pair=' + coin + '-' + exchange, json: true }, function (error, response, body) {
        if (error) {
            ...
            summary['volume'] = body.return[i]['volume_' + coin].toFixed(3);
            summary['volume_btc'] = body.return[i]['volume_' + exchange].toFixed(3);
            sleep9s;
            request({ uri: base_url + '/' + coin + '-' + exchange + '.json', json: true }, function (error, response, body) {
                if (error) {
                    return cb(error, null);
                } else if (body != undefined) {
                    summary['bid'] = body.ticker['buy'].toFixed(3);
                    summary['ask'] = body.ticker['sell'].toFixed(3);
                    summary['high'] = body.ticker['high'].toFixed(3);
                    summary['low'] = body.ticker['low'].toFixed(3);
                    summary['last'] = body.ticker['lastprice'].toFixed(3);
            ...
            if (tTrades == "No trade history for this period") {
                ...
                for (var i = 0; i < tTrades.length; i++) {
                    var Trade = {
                        ...
                        price: parseFloat(tTrades[i].rate).toFixed(3),
                        total: (parseFloat(tTrades[i].amount).toFixed(3) * parseFloat(tTrades[i].rate)).toFixed(3),
        ...
        if (body != undefined) {
            ...
            for (Data in orders.Data) {
                var order = {
                    ...
                    amount: parseFloat(orders.Data[Data].amount.toFixed(3)),
                    price: parseFloat(orders.Data[Data].price).toFixed(3),
                    total: (parseFloat(orders.Data[Data].amount) * parseFloat(orders.Data[Data].price)).toFixed(3)
                }
                if (order.otype == 'buy') {
                    buys.push(order);
                } else {
