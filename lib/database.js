...
module.exports = {
  ...
  update_richlist: function(list, cb){
    if(list == 'received') {
      ...
      Address.find({}).sort({balance: 'desc'}).limit(100 + Object.keys(settings.labels).length).exec(function(err, addresses){
        Richlist.update({coin: settings.coin}, {
          balance: addresses,
        }, function() {
          return cb();
  ...
  get_distribution: function(richlist, stats, cb){
    ...
    lib.syncLoop(richlist.balance.length, function (loop) {
      ...
      // calculate locked
      if (settings.labels[richlist.balance[i].a_id]) {
        distribution.locked.percent = distribution.locked.percent + percentage;
        distribution.locked.total = distribution.locked.total + (richlist.balance[i].balance / 100000000);
      }
      else {
        count++;
      ...
      // distribution.t_101plus.total = parseFloat(distribution.supply - distribution.t_76_100.total - distribution.t_51_75.total - distribution.t_26_50.total - distribution.t_1_25.total).toFixed(3);
      // distribution.t_1_25.percent = parseFloat(distribution.t_1_25.percent).toFixed(2);
      // distribution.t_1_25.total = parseFloat(distribution.t_1_25.total).toFixed(3);
      // distribution.t_26_50.percent = parseFloat(distribution.t_26_50.percent).toFixed(2);
      // distribution.t_26_50.total = parseFloat(distribution.t_26_50.total).toFixed(3);
      // distribution.t_51_75.percent = parseFloat(distribution.t_51_75.percent).toFixed(2);
      // distribution.t_51_75.total = parseFloat(distribution.t_51_75.total).toFixed(3);
      // distribution.t_76_100.percent = parseFloat(distribution.t_76_100.percent).toFixed(2);
      // distribution.t_76_100.total = parseFloat(distribution.t_76_100.total).toFixed(3);
